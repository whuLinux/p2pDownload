...
TCPSocketUtil::~TCPSocketUtil()
{
    ...
    for (QMap<qint32, Client *>::iterator clientsMapIt = this->clientsMap.begin(); clientsMapIt != this->clientsMap.end(); clientsMapIt++) {
        ...
        closeConnection(clientsMapIt.key());
        closeFileConnection(clientsMapIt.key());
...
bool TCPSocketUtil::closeConnection(qint32 partnerId)
{
    ...
    if (!this->partnerP2PConnections.contains(partnerId)) {
        qDebug() << "TCPSocketUtil::closeConnection " << "错误尝试关闭和伙伴客户端之间不存在的TCP连接的P2P辅助 " << "partnerId " << partnerId << endl;
        return false;
    }
...
bool TCPSocketUtil::closeFileConnection(qint32 partnerId)
{
    ...
    if (!this->partnerFileP2PConnections.contains(partnerId)) {
        qDebug() << "TCPSocketUtil::closeFileConnection " << "错误尝试关闭和伙伴客户端之间不存在的TCP连接的P2P辅助 " << "partnerId " << partnerId << endl;
        return false;
    }
...
bool TCPSocketUtil::disConnectToFileFriend(qint32 friendId)
{
    ...
    if (this->fileGuests[friendId] == nullptr) {
        qDebug() << "TCPSocketUtil::disConnectToFileFriend " << "指定连接该朋友客户端的P2PTcpSocket对象的P2P辅助已经关闭 " << "friendId " << friendId << endl;
        return false;
    }
    ...
    if (!this->p2pFileGuests.contains(friendId)) {
        qDebug() << "TCPSocketUtil::disConnectToFileFriend " << "指定连接该朋友客户端的P2PTcpSocket对象的P2P辅助尚未建立 " << "friendId " << friendId << endl;
        return false;
    }
...
bool TCPSocketUtil::disConnectToPartner(qint32 partnerId)
{
    ...
    if (this->partnerConnections[partnerId] == nullptr) {
        qDebug() << "TCPSocketUtil::disConnectToPartner " << "指定连接该伙伴客户端的P2PTcpSocket对象已经关闭 " << "partnerId " << partnerId << endl;
        return false;
    }
...
bool TCPSocketUtil::disConnectToFilePartner(qint32 partnerId)
{
    ...
    if (this->partnerFileConnections[partnerId] == nullptr) {
        qDebug() << "TCPSocketUtil::disConnectToFilePartner " << "指定连接该伙伴客户端的P2PTcpSocket对象已经关闭 " << "partnerId " << partnerId << endl;
        return false;
    }
